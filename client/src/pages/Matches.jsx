...
function UserSkills({ user }) {
  ...
  return (
    <div className="mt-4 flex flex-wrap gap-2">
      {skills.length ? skills.map(({ skill, level }) => <span className="rounded-full bg-white/10 px-3 py-1 text-xs text-slate-300" key={`${skill}-${level}`}>{skill} / {level}</span>) : <span className="text-sm text-text-muted">No skills listed</span>}
    </div>
  )
}
...
function SharedSkills({ match, currentUserId }) {
  ...
  return <div className="mt-4 grid gap-2 text-xs text-text-muted sm:grid-cols-2"><p><span className="font-semibold text-slate-200">Learn from them:</span> {canLearn.join(', ') || 'No overlap yet'}</p><p><span className="font-semibold text-slate-200">Teach them:</span> {canTeach.join(', ') || 'No overlap yet'}</p></div>
}
  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <main className="mx-auto max-w-6xl px-6 py-12 md:px-10 md:py-16">
        <div><p className="eyebrow text-amber-400">Your network</p><h1 className="font-display mt-4 text-5xl font-bold tracking-tight text-text-primary md:text-7xl">Matches</h1><p className="mt-5 max-w-xl text-lg leading-8 text-text-muted">Respond to new introductions and keep track of the people you connect with.</p></div>
        ...
        {isLoading ? <div className="py-20 text-center text-text-muted">Loading your network...</div> : <div className="mt-12 space-y-16">
          <section aria-labelledby="requests-heading"><div className="flex items-baseline justify-between gap-4"><h2 className="font-display text-3xl font-bold text-text-primary" id="requests-heading">Pending requests</h2><span className="text-sm font-semibold text-text-muted">{pending.length}</span></div>
            {pending.length === 0 ? <p className="mt-5 text-text-muted">No new requests right now.</p> : <div className="mt-6 grid gap-5 md:grid-cols-2">{pending.map((match) => <motion.article animate={{ opacity: 1, y: 0 }} className="glass-card p-6" initial={{ opacity: 0, y: 12 }} key={match._id} transition={{ duration: 0.3 }}><div className="flex items-start justify-between gap-4"><div className="flex items-center gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-amber-400 font-display text-lg font-bold text-surface">{match.userA?.name?.slice(0, 1).toUpperCase()}</div><div><h3 className="font-display text-xl font-bold text-text-primary">{match.userA?.name}</h3><p className="text-sm text-text-muted">wants to connect</p></div></div><span className="font-display text-2xl font-bold text-amber-300">{match.matchScore}%</span></div><UserSkills user={match.userA} /><div className="mt-6 flex gap-3"><button className="primary-button" disabled={respondingId === match._id} onClick={() => respond(match._id, 'accept')} type="button">{respondingId === match._id ? 'Saving...' : 'Accept'}</button><button className="rounded-lg border border-white/15 px-5 py-2 text-sm font-semibold text-slate-300 transition hover:border-red-400/40 hover:text-red-400" disabled={respondingId === match._id} onClick={() => respond(match._id, 'reject')} type="button">Reject</button></div></motion.article>)}</div>}
          </section>
          ...
          <section aria-labelledby="connections-heading"><div className="flex items-baseline justify-between gap-4"><h2 className="font-display text-3xl font-bold text-text-primary" id="connections-heading">My Connections</h2><span className="text-sm font-semibold text-text-muted">{connections.length}</span></div>
{connections.length === 0 ? <p className="mt-5 text-text-muted">Accepted connections will appear here.</p> : <div className="mt-6 grid gap-5 md:grid-cols-2">{connections.map((match) => {
    const matchedUser = MatchedUser({ match, currentUserId: user.id })
    return (
      <motion.article
      ...
      >
        <div className="flex items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              <Link className="block truncate font-display text-xl font-bold text-text-primary hover:underline" to={`/profile/${matchedUser?._id}`}>{matchedUser?.name}</Link>
        ...
        {reportingId === match._id && (
          <div className="mt-4 rounded-lg border border-white/10 bg-white/5 p-4">
            <p className="text-sm font-semibold text-slate-200">Report {matchedUser?.name}</p>
            ...
            {reportMessage && <p className="mt-2 text-sm text-green-400">{reportMessage}</p>}
          </div>
        )}
